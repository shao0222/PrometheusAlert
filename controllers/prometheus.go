...
type Annotations struct {
	...
	Yzjurl string `json:"yzjurl"` //增加云之家告警支持
}
type Alerts struct {
	Status       string
...
func SendMessageP(message Prometheus, logsign string) string {
	...
	var ddtext, yzjtext, fstext, wxtext, MobileMessage, PhoneCallMessage, titleend string
	//对分组消息进行排序
	...
	for _, RMessage := range AlerMessage {
		...
		if RMessage.Status == "resolved" {
			...
			yzjtext = "[" + Title + "Prometheus" + titleend + "]\n" + RMessage.Labels.Alertname + "\n" + "告警级别：" + AlertLevel[nLevel] + "\n" + "故障主机IP：" + RMessage.Labels.Instance + "\n" + RMessage.Annotations.Description
		...
		//发送消息到云之家
		if RMessage.Annotations.Yzjurl == "" {
			url := beego.AppConfig.String("yzjurl")
			PostToYunZhiJia(yzjtext, url, logsign)
		} else {
			Yzjurl := strings.Split(RMessage.Annotations.Yzjurl, ",")
			for _, url := range Yzjurl {
				PostToYunZhiJia(yzjtext, url, logsign)
			}
		}
		//发送消息到微信
